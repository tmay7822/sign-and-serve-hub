...
const GoogleReviewsSection = () => {
  const { getFeaturedReviews, averageRating, totalReviews, gmbProfileUrl, gmbViewUrl } = useGoogleReviews();
            ...
            <Card key={review.id} className="border border-border hover:shadow-lg transition-all duration-300">
              <CardContent className="p-6">
                <div className="flex items-start gap-3 mb-4">
                  <div className={`${avatarColors[index % avatarColors.length]} h-10 w-10 rounded-full flex items-center justify-center text-white font-semibold text-lg flex-shrink-0`}>
                    {review.reviewerInitial}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-foreground truncate">{review.reviewerName}</p>
                    <p className="text-sm text-muted-foreground">{formatRelativeDate(review.date)}</p>
                  </div>
                </div>
